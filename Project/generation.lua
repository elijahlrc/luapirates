...
--[[This code has some chalenges left in it, mainly that we need to know if the player is spaned in a large enough ocean.
--]]
-- Size of Map
local map_scale =11
-- Smoothness
...
function generate_map()--must be global
	...
	local function interpolate(a,b,c,d,delta_x,delta_y) -- takes 4 points and a delta x and delta y from top left point.
		return a*(1-delta_x)*(1-delta_y)+b*delta_x*(1-delta_y)+c*(1-delta_x)*(delta_y)+d*delta_x*delta_y
	end

	local function octave(octave_resolution,point_list,temp_list)-- takes the resolution and returns a smoothed noise array
		local value = 0
		local res_list = {}
		local oct_res_x
		local oct_res_y
	...
	local function perlin(octaves,point_list,temp_list) -- x,y must be powers of 2
		local weight = 0
	...
	local function smooth(factor,point_list)
		for k=1,factor do
...
function handle_objects(map_image,value,i,j)--generates towns, forests, seaweed, docs, etc
	local function maketown(i,j,size,map_image,value)--makes town positions and graphics, not statistics or items or stores or anythang
		local size = 0
	...
	local function MakeTownStats(size,pos)--makes town prices/goods
	...
	local function makedock(i,j,map_image,value)--generates doc which changes dirction ocationaly and has a random lenth
	...
	local function makeforest(i,j,map_image,value,max_size)
		local size = 0
		local rad = math.sqrt(max_size)
		local di = 0
		local dj = 0
	...
	--function defs above this line
	--calls below this line
	local height
	local obj
	...
	--[[
	This definitey needs some work because all towns should have docs.
	--]]
